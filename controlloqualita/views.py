...
from .helpers import build_kpi_structures, apply_filters
from .services.quality_report import get_categoria_from_servizio, calcola_punteggio_dynamic
from controlloqualita.helpers import _pack_trend   

@login_required
def kpi_dashboard(request):
    """
    Rende la pagina HTML che conterrà form & grafici dinamici.
    Usiamo lo stesso DashboardFilterForm.
    """
    form = DashboardFilterForm(request.GET or None)
    return render(request, "controlloqualita/kpi_dashboard.html", {
        "form": form,
    })
# --- endpoint JSON ---------------------------------------------------------
@login_required
def kpi_data(request):
    """
    Restituisce i KPI in formato JSON.
    GET accetta gli stessi parametri di dashboard (date_from, date_to, ecc.).
    """
    form = DashboardFilterForm(request.GET or None)
    qs   = ImportRecord.objects.all()
    if form.is_valid():
        qs = apply_filters(qs, form.cleaned_data)

    # ---------- calcolo delle “buckets” (= 4 date equidistanti) ----------
    # usiamo la data di scadenza se presente, altrimenti data_evasione
    from controlloqualita.helpers import parse_data_ddmmyyyy_hhmmss, parse_data_aaaammgg
    dates = []
    for r in qs:
        ds = parse_data_ddmmyyyy_hhmmss(r.raw_data.get('DataScadenza', ''))
        if not ds:
            ds = parse_data_aaaammgg(r.raw_data.get('DataEvasione', '') or '')
        if ds:
            dates.append(ds)
    if not dates:                       # nessuna data → evitiamo crash
        buckets = []
    else:
        start, end = min(dates), max(dates)
        span       = (end-start).days or 1           # evita div/0
        step       = span/3.0                        # 4 punti = 3 intervalli
        buckets = [
            (start + datetime.timedelta(days=round(step*i))).strftime("%d/%m/%Y")
            for i in range(4)
        ]

    # ---------- costruzione JSON KPI -------------------------------------
    data_json = build_kpi_structures(
        qs,
        get_categoria_from_servizio,
        lambda rec, cat: calcola_punteggio_dynamic({
            "categoria": cat,
            "dettagli" : rec.build_det()    # metodo definito sul model
        })
    )

    # tramite il nuovo helper _pack_trend (già importato in helpers.py)
    data_json["trend"] = _pack_trend(data_json.pop("trend_raw"), buckets)
    print('buuuuu')
    print(data_json.pop("trend"))
    #input()

    # ---------------------------------------------------------------------
    return JsonResponse(data_json, safe=True)
...
def export_to_xlsx(request, queryset):
    ...
    response['Content-Disposition'] = 'attachment; filename="esportazione_base_dati.xlsx"'
    return response
...
def dashboard(request):
    ...
    qs = qs.order_by('data_evasione', 'id')
